...
OUTPUT_RESULT = "./output/gender_age_rforest.csv"
...
def predict(csv_train, csv_test):
    ...
    df_train = df_train[['S_Gender', 'S_AgeFill', 'S_AgeNull']]
    df_test = df_test[['S_Gender', 'S_AgeFill', 'S_AgeNull']]
    survival = df_survived.values
    train = df_train.values
    test = df_test.values

    forest = RandomForestClassifier(n_estimators = 100)
    forest = forest.fit(train, survival)
